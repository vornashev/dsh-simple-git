...
function parsePayload(payload) {
	...
	const value = payload;
	if (typeof value.workspaceId !== "string" || value.message !== void 0 && typeof value.message !== "string" || value.url !== void 0 && typeof value.url !== "string") return void 0;
	return {
...
function formatGitError(command, exitCode, stderr, stdout = "") {
	...
	if (detail !== "") return detail.slice(0, 1e3);
...
async function probeGit(shell, path, command, signal) {
	const result = await executeGit(shell, path, `${command}; exit 0`, void 0, signal);
	if (result.exitCode !== 0) throw new Error(sanitizeError(formatGitError(command, result.exitCode, result.stderr.text, result.stdout.text)));
...
async function isRepository(shell, path, signal) {
	await runGit(shell, path, "git --version", void 0, signal);
	const lines = (await probeGit(shell, path, "git rev-parse --is-inside-work-tree --show-prefix", signal)).split(/\r?\n/);
...
async function status(shell, workspaceId, path, signal) {
	if (!await isRepository(shell, path, signal)) return {
		workspaceId,
		initialized: false
	};
	...
	ctx.connection.rpc.handle("/simple-git", async (endpoint, rawPayload, signal) => {
		...
		return withWorkspaceLock(payload.workspaceId, async () => {
			try {
				...
				if (endpoint === "init") {
					if (!await isRepository(ctx.shell, workspace.path, signal)) await runGit(ctx.shell, workspace.path, "git init", void 0, signal);
				...
				if (endpoint === "remote") {
					...
					await runGit(ctx.shell, workspace.path, `git remote add origin ${remote}`, void 0, signal);
					return ok(await status(ctx.shell, payload.workspaceId, workspace.path, signal));
				}
				if (endpoint === "commit") {
					if (!await isRepository(ctx.shell, workspace.path, signal)) return fail("Сначала инициализируйте Git-репозиторий.");
				...
				if (endpoint === "push") {
					...
					if (snapshot.upstreamConfigured) await runGit(ctx.shell, workspace.path, "git push", void 0, signal);
					else if (snapshot.originConfigured) await runGit(ctx.shell, workspace.path, "git push -u origin HEAD", void 0, signal);
...
//#endregion
export { apply, formatGitError, inject, normalizeGitHubRemote, parseNumstat, parseStatus, sanitizeError };
