...
export function formatGitError(command, exitCode, stderr, stdout = '') {
    ...
    if (lower.includes('not a git repository'))
        return 'Эта папка не является Git-репозиторием.';
    if (lower.includes('author identity unknown') || lower.includes('please tell me who you are'))
        return 'Git не знает имя и email автора. Настройте user.name и user.email.';
    if (lower.includes('no configured push destination'))
        return 'Для репозитория не настроен удалённый репозиторий (remote).';
    if (lower.includes('has no upstream branch'))
        return 'Для текущей ветки не настроена удалённая upstream-ветка.';
    if (lower.includes('non-fast-forward') || lower.includes('[rejected]') || lower.includes('fetch first') || lower.includes('remote contains work') || lower.includes('rejected'))
        return 'Push отклонён: удалённая ветка содержит изменения. Сначала выполните pull или rebase.';
    if (lower.includes('src refspec') && lower.includes('does not match any'))
        return 'Не удалось выполнить push: текущая ветка ещё не содержит коммитов или не существует.';
    if (lower.includes('could not read username') || lower.includes('authentication failed') || lower.includes('permission denied'))
        return 'Не удалось выполнить push: Git не смог пройти аутентификацию или у вас нет доступа к репозиторию.';
    if (detail !== '')
...
async function executeGit(shell, workdir, command, stdin, signal) {
...
async function probeGit(shell, path, command, signal) {
...
async function isRepository(shell, path, signal) {
...
async function hasOrigin(shell, path, signal) {
    return await probeGit(shell, path, 'git config --get remote.origin.url', signal) !== '';
}
async function hasUpstream(shell, path, signal) {
...
export function normalizeGitHubRemote(value) {
    ...
    if (input.length === 0)
        throw new Error('Укажите URL существующего репозитория GitHub.');
    if (input.length > MAX_REMOTE_URL)
        throw new Error(`URL репозитория должен быть не длиннее ${MAX_REMOTE_URL} символов.`);
...
async function status(shell, workspaceId, path, signal) {
    if (!await isRepository(shell, path, signal))
        return { workspaceId, initialized: false };
    ...
    ctx.connection.rpc.handle('/simple-git', async (endpoint, rawPayload, signal) => {
        ...
        return withWorkspaceLock(payload.workspaceId, async () => {
            try {
                ...
                if (endpoint === 'init') {
                    if (!await isRepository(ctx.shell, workspace.path, signal))
                        await runGit(ctx.shell, workspace.path, 'git init', undefined, signal);
                    return ok(await status(ctx.shell, payload.workspaceId, workspace.path, signal));
                }
                if (endpoint === 'remote') {
                    if (!await isRepository(ctx.shell, workspace.path, signal))
                        return fail('Сначала инициализируйте Git-репозиторий.');
                    if (await hasOrigin(ctx.shell, workspace.path, signal))
                        return fail('Remote origin уже настроен. Плагин не изменяет существующий remote автоматически.');
                ...
                if (endpoint === 'push') {
                    ...
                    if (!snapshot.initialized)
                        return fail('Сначала инициализируйте Git-репозиторий.');
                    if (snapshot.branch === '')
                        return fail('Нельзя настроить первый push из detached HEAD. Переключитесь на ветку.');
                    if (snapshot.upstreamConfigured)
                        await runGit(ctx.shell, workspace.path, 'git push', undefined, signal);
                    else if (snapshot.originConfigured)
                        await runGit(ctx.shell, workspace.path, 'git push -u origin HEAD', undefined, signal);
                    else
                        return fail('Сначала подключите GitHub-репозиторий как remote origin.');
                    return ok(await status(ctx.shell, payload.workspaceId, workspace.path, signal));
                }
