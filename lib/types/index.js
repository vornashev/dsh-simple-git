...
function gitArg(path) { return JSON.stringify(path); }
...
async function runGit(shell, workdir, command, stdin) {
    ...
    if (result.exitCode !== 0)
        throw new Error(result.stderr.text || `git command failed with exit code ${result.exitCode ?? 'null'}`);
...
async function status(shell, workspaceId, path) {
    ...
    for (const line of diff.trim().split(/\r?\n/)) {
        ...
        if (file !== undefined)
            stats.set(file, [Number(additions) || 0, Number(deletions) || 0]);
    }
...
function rootArg(path) { return gitArg(path); }
